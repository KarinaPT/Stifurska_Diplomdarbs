...
if (isset($_SESSION['user_name'])) {
    ...
    if (isset($_POST['update'])) {
        ...
        $kat_query = "SELECT `Kategorija_ID` FROM `kategorija` WHERE `Nosaukums_kategorija` = '" . $Nosaukums_kategorija . "'";
        $kat_result = mysqli_query($conn, $kat_query);
        $row = mysqli_fetch_assoc($kat_result);
        $Kategorija_ID = $row['Kategorija_ID'];

        $sublile_query = "SELECT `Kapakssadala_ID` FROM `k_apakssadala` WHERE `Nosaukums_sadala` = '" . $Nosaukums_sadala . "'";
        ...
        $query = "UPDATE `prece`
                LEFT JOIN `kategorija` ON `prece`.`ID_KApakssadala` = `kategorija`.`Kategorija_ID`
                LEFT JOIN `k_apakssadala` ON `prece`.`ID_Kategorija_KApakssadala` = `k_apakssadala`.`Kapakssadala_ID`
